...
- No comments in code. Names and structure say what code does; a test says why a non-obvious behavior must hold. Facts that only a comment used to carry go into a name, a test, or the commit body; history and external refs go there or in docs. `bun run lint` fails on any comment.
